...
namespace coda {
  format::format(const string &str)
      : value_(str), specifiers_(), currentSpecifier_(specifiers_.begin()) {
    initialize();
  }
  ...
  format::format(const format &other)
      : value_(other.value_), specifiers_(),
    ...
    advance(currentSpecifier_,
            distance(other.specifiers_.begin(),
                     SpecifierList::const_iterator(other.currentSpecifier_)));
  }
  ...
  format &format::operator=(const format &rhs) {
    ...
    // advance current position according to the other position
    advance(currentSpecifier_,
            distance(rhs.specifiers_.begin(),
                     SpecifierList::const_iterator(rhs.currentSpecifier_)));
  ...
  size_t format::specifiers() const {
    // the size of the specifier list minus any specifier arguments already
    // added
    return specifiers_.size() -
           distance(specifiers_.begin(),
                    SpecifierList::const_iterator(currentSpecifier_));
  }
  ...
  void format::unescape(ostream &buf, string::size_type start,
  ...
  format::operator string() { return str(); }
